...
static int cur_xpos = 0, cur_area = 0, cur_l = 0, cur_r = 0, cur_ballfound = 0;
...
extern "C" void vision_ui_update_values(int xpos, int area, int l_speed, int r_speed, float distance, int ballfound) {
	cur_xpos      = xpos;
	cur_area      = area;
	cur_l         = l_speed;
	cur_r         = r_speed;
	cur_distance  = distance;
	cur_ballfound = ballfound;
}

extern "C" void vision_ui_get_values(int * xpos, int * area, int * l_speed, int * r_speed, float * distance, int * ballfound) {
	*xpos      = cur_xpos;
	*area      = cur_area;
	*l_speed   = cur_l;
	*r_speed   = cur_r;
	*distance  = cur_distance;
	*ballfound = cur_ballfound;
}
